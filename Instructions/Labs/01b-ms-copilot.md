...
1. 다음과 같이 Edge 도구 모음의 **Copilot** 아이콘을 사용하여 Copilot 창을 엽니다.
    ...
    > **팁**: Microsoft Copilot은 지속적으로 개선됩니다. 사용자 인터페이스가 정확히 표시된 이미지와 일치하지 않을 수 있습니다.
...
1. **메일** 페이지에서 새 이메일을 만듭니다. **받는 사람** 상자에 자신의 이메일 주소를 입력하고 **제목**을 `Business funding meeting request`로 설정합니다.
1. Copilot 창에서 다음 프롬프트를 입력합니다.
